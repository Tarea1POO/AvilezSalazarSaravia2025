...
public class ConsolasDAO {
    ...
    public List<Consolas> obtenerTodos(){
        List<Consolas> consola = new ArrayList<>();
        String sql = "SELECT * FROM consolas";
        try(Statement stmt = connection.createStatement()){
            ResultSet resultado = stmt.executeQuery(sql);
            while(resultado.next()){
                consola.add(new Consolas(resultado.getInt("id_consola"),
    ...
    public void actualizarConsolas(Consolas consola){
        ...
        try(PreparedStatement stmt= connection.prepareStatement(sql)){
            ...
            stmt.setInt(3,consola.getAnio_lanzamiento());
            stmt.setInt(4,consola.getId_consola());
            stmt.executeUpdate();
        }catch(SQLException e){
            e.printStackTrace();
        }
